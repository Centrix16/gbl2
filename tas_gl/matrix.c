...
//	glPolygonMode(GL_FRONT_AND_BACK, GL_LINE);
	...
	while (!glfwWindowShouldClose(window)) {
		...
		glm_rotate_make(trans, time * M_PI / 4, (vec3){0.5f, 0.5f, 1.0f});
		glm_translate(trans, (vec3){0.5f, -0.5f, 0.0f});
		glm_rotate(trans, -1 * time * M_PI / 2, (vec3){0.0f, 0.0f, 1.0f});
		glm_scale(trans, (vec3){fmod(time,2), 0.5f, 0.5f});
		
		GLint transLoc = glGetUniformLocation(shaderProgram, "transform");
		glUniformMatrix4fv(transLoc, 1, GL_FALSE, (GLfloat*)trans);
