...
## Основные функции:
  ...
  + `while` -- цикл while
  + `for` -- цикл for
  + `str` -- базовые операции со строками, первый агремент операция (`get / set`), второй -- индекс, третий переменная, в случае если операция выбрана операция `set`, четвёртый аргумент означает символ, который вставлен в строку по соответствующему индексу
  + `val` -- меняет дерево, заменяя себя конкатенацией значений своих наследников (`val (1)(2)(3) --> 123, val (a)(.)(12) --> a.12`)
